...
"novo"
<?php
    if ( !isset($_REQUEST['cpf']) || !isset($_REQUEST['nome']) || !isset($_REQUEST['api']) ) {
    ...
    $sql_find = " select id from cliente where cpf = '".$cpf."'; ";
    $meta = $conexao->query( $sql_find );
    $cliente = $meta->fetch(2);
    
    if ( $cliente ) {
        $cliente_id = $cliente['id'];
    } else {
        $sql_insert = " insert into cliente (cpf, nome) values ('".$cpf."', '".$nome."'); ";
        $resultado = $conexao->exec($sql_insert);
    ...
    if ( $cliente_id ) {
        ...
        echo json_encode(['success' => false, 'error' => 'Failed to find or create client.']);
    }
?>